...
func runAllBenchmarks(g *graph.Graph) {
	...
	// Benchmark bidirectional vs unidirectional
	fmt.Println("\nBidirectional vs Unidirectional A*:\n")
	fmt.Println("Algorithm                        | Iterations | Avg Time | Speedup")
	fmt.Println("--------------------------------|------------|----------|--------")

	uniResult := benchmarkRoute(g, "Unidirectional", 43.73, 7.42, 43.74, 7.43, "car", 100)
	biResult := benchmarkBidirectional(g, "Bidirectional", 43.73, 7.42, 43.74, 7.43, 100)

	fmt.Printf("%-32s | %10d | %8.2fms | 1.00x\n",
		uniResult.Name, uniResult.Iterations,
		float64(uniResult.AvgTime.Microseconds())/1000.0)

	speedup := float64(uniResult.AvgTime) / float64(biResult.AvgTime)
	fmt.Printf("%-32s | %10d | %8.2fms | %.2fx\n",
		biResult.Name, biResult.Iterations,
		float64(biResult.AvgTime.Microseconds())/1000.0,
		speedup)
...
func benchmarkBidirectional(g *graph.Graph, name string, fromLat, fromLon, toLat, toLon float64, iterations int) BenchmarkResult {
	router := routing.NewRouter(g)
	...
	for i := 0; i < iterations; i++ {
		start := time.Now()
		_, err := router.FindRouteBidirectional(fromLat, fromLon, toLat, toLon)
		elapsed := time.Since(start)
